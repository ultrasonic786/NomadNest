...
function SearchTagsResults() {
  ...
  async function fetchSearchTag() {
    ...
    const url = `https://jobicy.p.rapidapi.com/api/v2/remote-jobs?count=50&tag=${query}`;
    const options = {
      method: 'GET',
      headers: {
    ...
    <div>
      {
        !globalContextItems.loading ? (
          <div className="gutter jobs-gutter">
            ...
            {
              globalContextItems.searchTagResults.length > 0 ?
                (
                  globalContextItems.searchTagResults.map((element) => (
...
export default SearchTagsResults